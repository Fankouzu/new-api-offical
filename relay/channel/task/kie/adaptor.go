...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (string, []byte, *dto.TaskError) {
	...
	if taskResp.Code != http.StatusOK {
		return "", nil, service.TaskErrorWrapper(fmt.Errorf("kie api error: %s", taskResp.Msg), strconv.Itoa(taskResp.Code), http.StatusBadRequest)
	}
...
func (a *TaskAdaptor) ConvertToOpenAIAsyncImage(originTask *model.Task) ([]byte, error) {
	out := map[string]any{
		"object":     "kie.image.generation.task",
...
func (a *TaskAdaptor) convertToRequestPayload(req *relaycommon.TaskSubmitReq, info *relaycommon.RelayInfo) (*createTaskRequest, error) {
	...
	if len(images) > 0 {
		...
		case ModelHappyHorseImageToVideo:
			input[cfg.ImageKey] = images[:1]
		default:
			if cfg.ImageKey != "" {
...
func resolveModelName(req *relaycommon.TaskSubmitReq, info *relaycommon.RelayInfo) string {
	...
	if shouldUseDefaultModel(modelName, info) {
		path := ""
		if info != nil {
			path = info.RequestURLPath
		}
		return DefaultModelForRequest(path, req.HasImage())
	}
	return modelName
}

func shouldUseDefaultModel(modelName string, info *relaycommon.RelayInfo) bool {
	...
	fallback := service.CoverTaskActionToModelName(constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeKieAI)), action)
	if modelName == fallback {
		return true
	}
	return action != "" && modelName == ChannelName+"_"+strings.ToLower(action)
}
